...
export async function GET(request) {
    ...
    if (!user.projects.freelancer.includes(projectId)) {
    ...
    await deleteDoc(doc(db, "freelancer", projectId));
    await deleteDoc(doc(db, "published_freelancer", projectId));
    const deleteProjectIndex = user.projects.freelancer.indexOf(projectId);
    user.projects.freelancer.splice(deleteProjectIndex, 1);
