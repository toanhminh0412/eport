...
export function EditablePortfolio1({ section, sectionInd }) {
    ...
    return (
        <div className="group relative">
            <button className="btn bg-blue-700 border-none z-40 absolute hover:bg-blue-900 top-[-35px] right-0 mr-7 hidden group-hover:block" onClick={()=>document.getElementById('delete_modal_portfolio1').showModal()}><i className="fa-solid fa-trash text-lg text-white"></i></button>
            <dialog id="delete_modal_portfolio1" className="modal modal-bottom sm:modal-middle">
                        ...
                        <div key={project.id}>
                            ...
                            <div className="modal ml-96 lg:ml-[32rem] mt-48 p-0 modal-middle">
                                <div className="modal-box w-10/12 max-w-3xl mr-96 mb-40">
                                    ...
                                    <Swiper>
                                        {project.images.map(image => <swiper-slide key={image.id}><Image width={200} height={200} src={image.src} alt="" className="h-full w-full object-contain brightness-75"/></swiper-slide>)}
                                    </Swiper>
                                    </div> : null}
